...
**DicisionTokenSale.sol [100] - [121]** - The function `buyTokens()` allows the owner of the `wallet` address to essentially create as many tokens as they like up to the token cap, for free. The owner of the `wallet` address may send ETH to an arbitrary address (or series of addresses), which in turn calls `buyTokens()`. The calling address will then be credited with tokens, whilst the ETH is immediately sent back to the `wallet` address and the process can repeat.
